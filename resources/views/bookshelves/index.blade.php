        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflowhidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray100">
                    <x-primary-button tag="a" href="{{ route('bookshelf.create') }}">Create Data
                        Bookshelf</x-primary-button>
                            ...
                            <tr>
                                ...
                                <td>
                                    <x-primary-button tag="a"
                                        href="{{ route('bookshelf.edit', $bookshelf->id) }}">Edit
                                    </x-primary-button>
                                    <x-danger-button x-data=""
                                        x-on:click.prevent="$dispatch('open-modal', 'confirm-bookshelf-deletion')"
                                        x-on:click="$dispatch('set-action', '{{ route('bookshelf.destroy', $bookshelf->id) }}')">{{ __('Delete') }}</x-danger-button>
                                </td>
                            </tr>
                        @endforeach
                    </x-table>
                    ...
                    <x-modal name="import-book" focusable maxWidth="xl">
                        <form method="post" action="{{ route('bookshelf.import') }}" class="p-6"
                            ...
                            <div class="max-w-xl">
                                <x-input-label for="cover" class="sr-only"
                                    value="File
                                 Import" />
